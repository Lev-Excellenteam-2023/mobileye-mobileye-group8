...
def helper_draw_circles(image, x_red, y_red, x_green, y_green):
    fig, ax = plt.subplots()
    ax.imshow(image)

    # Draw red circles around point pairs
    for x, y in zip(x_red, y_red):
        # circle = plt.Circle((x, y), radius=radius, color="red", fill=False)
        # ax.add_patch(circle)
        ax.plot(x, y, marker='+', markersize=10, color='red')

    # Draw red circles around point pairs
    for x, y in zip(x_green, y_green):
        # circle = plt.Circle((x, y), radius=radius, color="green", fill=False)
        # ax.add_patch(circle)
        ax.plot([x - 10, x + 10], [y - 10, y + 10], color='green', linewidth=1)
        ax.plot([x + 10, x - 10], [y - 10, y + 10], color='green', linewidth=1)
...
def pixel_is_red(hsv):
    h, s, v = hsv
    # return h*360 > 345 or h*360 < 30
    return (h * 360 > 345 or h * 360 < 50) and s > 0.2 and v > 0.75


def pixel_is_green(hsv):
    h, s, v = hsv
    # return 60 <= h*360 <= 160
    return 75 <= h * 360 <= 190 and s > 0.3 and v > 0.6


def is_white(normalized_rgb_color, threshold=0.1):
    r, g, b = normalized_rgb_color
    return abs(r - g) <= threshold and abs(r - b) <= threshold


def region_is_white(image, region):
    count_white = 0
    for y, x in region.coords:
        if is_white(image[y, x]):
            count_white += 1
    return 0.8 < (count_white / region.num_pixels)


def region_is_red(image, region):
    count_red = 0
    for y, x in region.coords:
...
def region_is_green(image, region):
    count_green = 0
    for y, x in region.coords:
        if pixel_is_green(image[y, x]):
            count_green += 1
    return 0.25 < (count_green / region.num_pixels)


def find_local_max_centers(image, binary_image, area_threshold, color):
...
def find_tfl_lights(c_image: np.ndarray, **kwargs) -> Dict[str, Any]:
    ...
    g = c_image[:, :, 1]  # Green channel
    r = c_image[:, :, 2]  # Red channel

    # Apply blur to reduce small noises and smoothen image
    sigma = 1.0
    blur_red = filters.gaussian(r, sigma=sigma)
    # plt.imshow(blur_red)
    blur_green = filters.gaussian(g, sigma=sigma)
    # plt.imshow(blur_green)

    # Apply maximum filter to enhance relevant areas
    red_max = maximum_filter(blur_red, size=5)
    # plt.imshow(red_max)
    green_max = maximum_filter(blur_green, size=5)
    ...
    # kernel = [[-0.065, -0.065, -0.065, -0.065, -0.065],
    #           [-0.065, 0.05, 0.4, 0.05, -0.065],
    #           [-0.065, 0.4, 0.4, 0.4, -0.065],
    #           [-0.065, 0.05, 0.4, 0.05, -0.065],
    #           [-0.065, -0.065, -0.065, -0.065, -0.065]]
    #
    # convolved_red = convolve(r, kernel, mode='same')
    # plt.imshow(convolved_red)
    # convolved_green = convolve(g, kernel, mode='same')
    # plt.imshow(convolved_green)
    #
    # red_sub = np.subtract(red_max, convolved_red)
    # plt.imshow(red_sub)
    ...
    y_red = []
    for c in centroids:
        # if is_green_or_red(c_image, c[0], c[1]) > 0:
        x_red.append(c[1])
        y_red.append(c[0])
    ...
    # helper_draw_circles(c_image, x_red, y_red, x_green, y_green)
